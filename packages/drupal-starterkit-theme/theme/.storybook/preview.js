...
import { getBreakpoints, attachLibraries } from './utils.js'

const libraryJson = require.context('../dist', true, /.*\.libraries\.json$/, 'eager').keys()[0]
const url = CONFIG_TYPE === 'PRODUCTION'
  ?  `/../${libraryJson}`
  : `http://localhost:5173/dist/${libraryJson}`

attachLibraries(url, CONFIG_TYPE)
